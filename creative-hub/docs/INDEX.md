# Documentation Index

## Quick Links

| Need | Read |
|------|------|
| **What's working right now?** | [STATUS_2025-12-28.md](./STATUS_2025-12-28.md) |
| **How do we launch & grow?** | [GTM_STRATEGY.md](./GTM_STRATEGY.md) **(NEW!)** |
| **How to set up?** | [SETUP_GUIDE.md](./SETUP_GUIDE.md) |
| **How does the music intelligence work?** | [HIT_DNA_ARCHITECTURE.md](./HIT_DNA_ARCHITECTURE.md) |
| **How do we make money?** | [MONETIZATION_ARCHITECTURE.md](./MONETIZATION_ARCHITECTURE.md) |
| **What does each feature cost?** | [UNIT_ECONOMICS.md](./UNIT_ECONOMICS.md) |
| **Why Turso + Qdrant?** | [DATABASE_OPTIONS.md](./DATABASE_OPTIONS.md) |

## Also See

- **Skills Library**: `../skills/INDEX.md` - Production techniques, API guides
- **Lyric Pipeline**: `../scripts/lyric-pipeline/README.md` - Python tools

## Document Purposes

### GTM_STRATEGY.md **(NEW!)**
Complete go-to-market strategy:
- Distribution channels (Telegram, MCP, Product Hunt)
- Launch timeline (Week 1-4)
- Value capture opportunities
- Competitive positioning
- Growth metrics and targets

### STATUS_2025-12-28.md
**Read this first tomorrow.** Shows what's live, credentials, next steps.

### SETUP_GUIDE.md
How to set up Turso, Qdrant, and run migrations from scratch.

### HIT_DNA_ARCHITECTURE.md
Deep dive on the music intelligence system:
- CLAP audio embeddings
- Lyric pattern extraction
- Learning loop design
- Multi-vector storage

### MONETIZATION_ARCHITECTURE.md
Business model and payment flows:
- Tier pricing (Free/Creator/Studio)
- Revenue streams (Stars, TON, x402)
- Feature gating

### UNIT_ECONOMICS.md
Cost tracking and profitability analysis:
- Per-track generation costs
- Break-even calculations
- Margin targets

### DATABASE_OPTIONS.md
Why we chose Turso + Qdrant over alternatives. Reference for future decisions.

---

*Updated: 2025-12-30*

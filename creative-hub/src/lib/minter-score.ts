...
// Import scammer data directly (works with Vercel bundler)
// This file is generated by scripts/build-ton-labels.py
import scammerData from "../../data/scammer-addresses.json";

// ============================================
// TON Labels Integration (Scammer Detection)
// ============================================
...
const scammerAddresses = scammerData as Record<string, TonLabel>;

/**
 * Check if an address is a known scammer
 * Uses bundled scammer-addresses.json for Vercel compatibility
 */
export function checkTonLabel(address: string): TonLabel | null {
  return scammerAddresses[address] || null;
}
...
export function isKnownScammer(address: string): boolean {
  return address in scammerAddresses;
}
...
export function getScammerCount(): number {
  return Object.keys(scammerAddresses).length;
}
...
export function getAddressLabels(address: string): {
  ...
  const label = scammerAddresses[address];
  ...
  // All entries in scammer-addresses.json are scammers
  riskFlags.push(`🚨 KNOWN SCAMMER: ${label.label} - ${label.comment || label.description}`);

  if (label.subcategory === "drainer") {
    riskFlags.push("⚠️ DRAINER CONTRACT: Will drain your wallet!");
  }

  if (label.tags?.includes("suspicious")) {
    riskFlags.push("⚠️ SUSPICIOUS: Flagged for suspicious activity");
  }

  return {
    isLabeled: true,
    isScammer: true, // All in this DB are scammers
    isCEX: false,
    isBot: false,
    label,
    riskFlags,
...
export async function calculateMinterScore(
  ...
  // Check ton-labels for known scammer FIRST (instant disqualification)
  ...
  // CRITICAL: If known scammer, override score to 0
  if (addressLabels.isScammer) {
    rawScore = 0;
  } else if (addressLabels.isBot) {
    rawScore = Math.min(rawScore, 20); // Cap at 200/1000 for bots
  }
  ...
  // Prepend scammer warnings (most important)
  if (addressLabels.riskFlags.length > 0) {
    warnings.unshift(...addressLabels.riskFlags);
  }

  const recommendation = addressLabels.isScammer
    ? "🚨 CONFIRMED SCAMMER - This address is in the TON community scammer database. DO NOT INTERACT."
    : generateRecommendation(finalScore, warnings);

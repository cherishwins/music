    ...
    tools: [
      {
        name: "generate_music",
        description:
          "Generate AI music track. Returns base64 audio. Costs $0.50 per track via x402 micropayment.",
        inputSchema: {
          type: "object",
          properties: {
            prompt: {
      ...
      },
      {
        ...
        description:
          "Generate viral-optimized lyrics using Loop-First design. Analyzes 4,800+ viral tracks for patterns. Free to generate, costs $0.50 to create music.",
        inputSchema: {
          type: "object",
          properties: {
            theme: {
      ...
      },
      {
        ...
        description:
          "Analyze lyrics for viral potential. Returns score 0-100 based on repetition ratio, hook density, and TikTok optimization.",
        inputSchema: {
          type: "object",
          properties: {
            lyrics: {
...
server.setRequestHandler(CallToolRequestSchema, async (request): Promise<CallToolResult> => {
  ...
  try {
    switch (name) {
      case "generate_music": {
        const { prompt, style = "phonk", duration_seconds = 60 } = args as {
          prompt: string;
          style?: string;
      ...
      case "generate_viral_lyrics": {
        const {
          theme,
          style = "phonk",
          target_viral_score = 50,
          include_music = false,
        } = args as {
          theme: string;
          style?: string;
          target_viral_score?: number;
      ...
      case "get_styles": {
        ...
        for (const style of VIRAL_STYLES) {
          text += `- **${style}**: ${MUSIC_STYLES[style as keyof typeof MUSIC_STYLES]}\n`;
        }
        text += "\n## All Styles\n";
